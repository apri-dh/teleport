...
func StrongValidateRole(role *scopedaccessv1.ScopedRole) error {
	...
	if mode := role.GetSpec().GetSsh().GetSessionRecordingMode(); mode != "" {
	...
	if mode := role.GetSpec().GetDefaults().GetSessionRecordingMode(); mode != "" {
		switch constants.SessionRecordingMode(mode) {
		case constants.SessionRecordingModeStrict, constants.SessionRecordingModeBestEffort:
		default:
	...
	if ehr := role.GetSpec().GetSsh().GetEnhancedRecording(); len(ehr) > 0 {
		for _, e := range ehr {
			if e == constants.EnhancedRecordingCommand ||
				e == constants.EnhancedRecordingDisk ||
				e == constants.EnhancedRecordingNetwork {
				continue
			}
			return trace.BadParameter("invalid value for role option enhanced_recording: %v", e)
		}
	}
