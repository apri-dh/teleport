...
func TestServiceScopedAccess(t *testing.T) {
	...
	t.Run("read allowed despite scope pin", func(t *testing.T) {
		service, _ := newServiceWithScopedAuthorizer(t, newScopedAuthorizer(t))
	...
	t.Run("write denied for scoped identity", func(t *testing.T) {
		service, emitter := newServiceWithScopedAuthorizer(t, newScopedAuthorizer(t))
...
func newScopedAuthorizer(t *testing.T) fakeScopedAuthorizer {
	t.Helper()
